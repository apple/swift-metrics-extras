...
- Proposal: SSM-0001
- Author(s): [Author 1](https://github.com/kukushechkin)
...
This proposal aims to change the public interface of the package to bring it inline with the latest best practices in the Swift ecosystem and reduce the API to a minimum focused on automatic background gathering of system metrics for the supported platforms.
...
The proposed interface also makes the package more testable, as there is no dependency on the global state.
...
With configuration represented by two structure `SystemMetricsMonitor.Configuration` and `SystemMetricsMonitor.Configuration.Labels`:
